...
export type UnitCheckResult = {
  /** True if the unit is acceptable for this commodity. */
  allowed: boolean;
  /**
   * Short machine-readable reason token, e.g.
   *   "unit_allowed" | "no_specific_rule" | "critical_unit_violation:banana_kg"
   */
  reason: string;
  /** The unit string normalised to lowercase slash-notation (e.g. "pkr/40kg"). */
...
function hasMissingUnit(unit: string): boolean {
  return !String(unit ?? "").trim();
}

function hasMixedUnitPhrase(unit: string): boolean {
...
function hasAmbiguousUnit(unit: string): boolean {
  const normalized = normalizeUnitToken(unit);
 ...
 */
const COMMODITY_UNIT_ALLOW_LIST: Record<string, string[]> = {
...
// ---------------------------------------------------------------------------
// Critical combos that are clearly impossible (hard reject regardless of
// whether the commodity is in the allow-list)
// ---------------------------------------------------------------------------

type CriticalViolation = {
  commodityKeywords: string[];
  forbiddenUnitKeywords: string[];
  reason: string;
};

const CRITICAL_VIOLATIONS: CriticalViolation[] = [
  {
 ...
 */
function extractCommodityKey(commodityName: string): string | null {
 ...
 */
export function checkUnitForCommodity(
  unit: string,
 ...
 */
export function isCriticalUnitViolation(commodityName: string, unit: string): boolean {
...
function hasUnitToken(source: string, token: "100kg" | "50kg" | "40kg" | "kg" | "dozen" | "tray" | "crate"): boolean {
  const normalized = normalizeUnitToken(source);
  return normalized.includes(token);
}

export function detectRawUnitConflict(
  normalizedUnit: string,
  rawPriceText: string,
): string | null {
  ...
  const hasBulkSignal =
    raw.includes("100 kg") || raw.includes("100kg") ||
  ...
  const hasCountSignal =
    raw.includes("dozen") || raw.includes("doz") || raw.includes("tray") ||
...
export function sanityRejectReason(
  commodityName: string,
  normalizedUnit: string,
  price: number,
): string | null {
  ...
  if (!Number.isFinite(value) || value <= 0) return "invalid_price";
