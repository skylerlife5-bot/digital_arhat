...
// ---------------------------------------------------------------------------
export const AMIS_COMMODITY_TO_APP_KEY: Record<string, string> = {
  "Wheat": "wheat",
...
function resolveColumnIndexes(headerCells: string[]): {
  cityIdx: number;
  minIdx: number;
  maxIdx: number;
  fqpIdx: number;
  ...
  for (let i = 0; i < headerCells.length; i += 1) {
    ...
    if (cityIdx === -1 && /(market|city|mandi|district)/.test(token)) cityIdx = i;
    if (minIdx === -1 && /\bmin(imum)?\b/.test(token)) minIdx = i;
    if (maxIdx === -1 && /\bmax(imum)?\b/.test(token)) maxIdx = i;
    if (fqpIdx === -1 && /(fqp|avg|average|rate|price)/.test(token)) fqpIdx = i;
    if (quantityIdx === -1 && /(quantity|arrival|arrivals)/.test(token)) quantityIdx = i;
  }

  return {cityIdx, minIdx, maxIdx, fqpIdx, quantityIdx};
}

function parseCityRows(
  html: string,
  context: {commodityId: number; commodityLabel: string; sourceUrl: string},
): ParsedCityRow[] {
  ...
  while ((match = trRegex.exec(html)) != null) {
    ...
    if (!/searchType=1&commodityId=\d+/i.test(trHtml)) continue;
