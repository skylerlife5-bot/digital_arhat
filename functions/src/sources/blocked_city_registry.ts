...
export type BlockerReasonCode =
  | "no_verified_structured_source"
  | "source_access_restricted"
  | "source_requires_auth_or_captcha"
  | "source_only_pdf_or_image"
  | "city_not_in_live_source_coverage";

export type BlockedCityEntry = {
  city: string;
...
const BLOCKED_CITY_LOOKUP: Record<string, Omit<BlockedCityEntry, "city">> = {
  Faisalabad: {
...
export function getBlockedCityRegistry(targets: TopPriorityMandiTarget[]): BlockedCityEntry[] {
  return targets
    .map((target) => {
    ...
    .filter((item): item is BlockedCityEntry => item != null);
}
