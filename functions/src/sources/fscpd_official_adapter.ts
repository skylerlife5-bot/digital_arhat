...
import {checkUnitForCommodity} from "./unit_rules";
...
const PUNJAB_DISTRICTS: Record<string, string> = {
  lahore:      "Lahore",
...
function canonicalDistrict(raw: string): string {
...
function toPositiveFinite(value: string): number | null {
 ...
 */
function parsePublishDate(html: string, now: Date): Date {
 ...
 */
function detectPageUnit(html: string): string {
  const lower = html.toLowerCase();
  if (lower.match(/per\s*40\s*kg|per\s*maund|pkr\s*\/\s*40\s*kg|rs\.\s*\/\s*40\s*kg/)) return "PKR/40Kg";
  if (lower.match(/per\s*100\s*kg|pkr\s*\/\s*100\s*kg/)) return "PKR/100Kg";
  if (lower.match(/per\s*kg|pkr\s*\/\s*kg/)) return "PKR/Kg";
 ...
 */
function parseHtmlTable(html: string, pageUnit: string): ParsedFscpdRow[] {
  const rows: ParsedFscpdRow[] = [];
  ...
  for (const tableHtml of tableMatch) {
    ...
    while ((trMatch = trRegex.exec(tableHtml)) != null) {
      ...
      const looksLikeHeader = cells.some((c) =>
        /district|city|commodity|item|min|max|rate|unit|sr/i.test(c),
      );

      if (!headerParsed && (isHeaderRow || looksLikeHeader)) {
        headerCols = cells.map((c) => c.toLowerCase().trim());
        headerCols.forEach((col, i) => {
          if (/^(sr|#|no\.?)$/.test(col)) return;
          if (/district|city|mandi/.test(col) && districtCol < 0) districtCol = i;
          if (/commodity|item|product|description|crop/.test(col) && commodityCol < 0) commodityCol = i;
          if (/min|minimum/.test(col) && minCol < 0) minCol = i;
          if (/max|maximum/.test(col) && maxCol < 0) maxCol = i;
          if (/\brate\b/.test(col) && rateCol < 0) rateCol = i;
          if (/unit/.test(col) && unitCol < 0) unitCol = i;
        });
        headerParsed = true;
        continue;
      }
      ...
      if (!headerParsed || (districtCol < 0 && commodityCol < 0)) {
        ...
        for (let i = 0; i < cells.length; i++) {
          if (!/^\d/.test(cells[i]) && cells[i].trim()) {
            ...
            else if (secondText < 0) { secondText = i; break; }
          }
        }
        if (firstText < 0 || secondText < 0) continue;
        ...
        if (numericCols.length >= 1) minCol = numericCols[0].i;
        if (numericCols.length >= 2) maxCol = numericCols[1].i;
      }

      const district = districtCol >= 0 ? cells[districtCol] ?? "" : "";
      const commodity = commodityCol >= 0 ? cells[commodityCol] ?? "" : "";
      if (!district.trim() || !commodity.trim()) continue;
      // Skip obvious header echoes in data rows
      if (/district|commodity|item|min|max/i.test(commodity)) continue;
      ...
      if (effectiveMin === null && effectiveMax === null) continue;
    ...
    if (rows.length > 0) break; // First table that yielded data wins
  }
...
async function fetchHtml(url: string): Promise<string> {
  ...
  for (let attempt = 0; attempt <= REQUEST_RETRIES; attempt++) {
    ...
    try {
      ...
      if (!resp.ok) throw new Error(`fscpd_http_${resp.status}`);
...
export class FscpdOfficialAdapter implements OfficialSourceAdapter {
  async fetchRows(context: AdapterContext): Promise<RawSourceRow[]> {
    const sourceUrl = String(
      process.env.FSCPD_OFFICIAL_SOURCE_URL ?? DEFAULT_URL,
    ).trim();
    ...
    for (const item of parsed) {
      ...
      if (!commodity || !district) continue;
      ...
      const unitCheck = checkUnitForCommodity(item.unit, commodity);
