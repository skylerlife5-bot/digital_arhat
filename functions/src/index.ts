...
import {
  computeFraudScore,
  computeImageHashFromStoragePath,
...
type CreateListingRequest = {
  listingData?: {
    ...
    promotionRequestedAt?: string | Date | Timestamp;
    promotionPaymentRequired?: boolean;
    promotionRequestedFeaturedListing?: boolean;
    promotionRequestedFeaturedAuction?: boolean;
    promotionPaymentReference?: string;
  ...
  mediaMetadata?: {
    verificationVideo?: {
      ...
      capturedAt?: string | Date | Timestamp;
      tag?: string;
...
function isUserBannedOrFlagged(userData: Record<string, unknown>): boolean {
...
function getRateLimitHourKey(now: Date): string {
...
function normalizePakPhone(value: unknown): string {
...
function hashPasswordSha256(value: string): string {
  return createHash("sha256").update(value, "utf8").digest("hex");
}

function emailFromPakPhone(normalizedPhone: string): string {
  const safe = normalizedPhone.replace(/[^0-9]/g, "");
  return `u_${safe}@digitalarhat.app`;
}

async function getUidFromBearerToken(req: Request): Promise<string | null> {
...
async function getDecodedTokenFromBearerToken(req: Request): Promise<admin.auth.DecodedIdToken | null> {
  const authHeader = String(req.headers.authorization || "").trim();
  if (!authHeader.toLowerCase().startsWith("bearer ")) return null;
...
export const establishCustomSession = onRequest({region: "asia-south1"}, async (req, res) => {
  setCorsHeaders(res);
...
function normalizeDetectedSide(value: string): "front" | "back" | "unknown" {
  const side = String(value || "").toLowerCase().trim();
  if (side === "front") return "front";
  if (side === "back") return "back";
  if (side.includes("front") || side.includes("frnt")) return "front";
  if (side.includes("back") || side.includes("rear")) return "back";
  return "unknown";
}
...
function toSuggestedAction(score: number): "review" | "approve_ok" | "reject_suspected" {
  if (score >= 80) return "reject_suspected";
  if (score >= 45) return "review";
  return "approve_ok";
}

function toBidAction(score: number): "allow" | "warn" | "hold" {
  if (score >= 92) return "hold";
...
function sanitizeReasonList(values: unknown, fallback: string): string[] {
  if (!Array.isArray(values)) return [fallback];
  const cleaned = values
    .map((item) => String(item || "").trim())
    .filter((item) => item.length > 0)
    .slice(0, 5);
  return cleaned.length > 0 ? uniqueStrings(cleaned) : [fallback];
}
...
async function enforceAiThrottle(params: {
  ...
  await db.runTransaction(async (tx) => {
    ...
    const count = toNumber(snap.get("count"), 0);
    if (count >= params.limit) {
      throw new HttpsError("resource-exhausted", "ai-throttled");
    }
...
type ListingAiResult = {
  ...
  aiReasons: string[];
  aiReasonsUrdu: string[];
  aiFlags: string[];
  aiSuggestedAction: "review" | "approve_ok" | "reject_suspected";
  aiConfidence: number;
};

async function evaluateListingRiskDocument(listingId: string): Promise<ListingAiResult> {
  ...
  const quantity = toNumber(listing.quantity, 0);
  ...
  const subcategory = String(
    listing.subcategory || listing.subCategory || listing.subcategoryLabel || "",
  ).trim();
  ...
  const marketAvg = await getMarketAverageRate(db, product, {
  ...
  let result: ListingAiResult = {
    ...
    aiFlags: uniqueStrings(heuristicFlags),
  ...
  try {
    ...
    const score = clampRisk(toNumber(parsed.aiRiskScore, heuristicScore));
    ...
    result = {
      ...
      aiReasonsUrdu: sanitizeReasonList(
        parsed.aiReasonsUrdu,
        "دستی جائزہ تجویز کیا جاتا ہے۔",
      ),
      aiFlags: sanitizeReasonList(parsed.aiFlags, "none"),
      aiSuggestedAction: action,
      aiConfidence: Math.max(0, Math.min(1, toNumber(parsed.aiConfidence, 0.6))),
    };
  } catch (_) {
...
async function callGeminiText(params: {
  ...
  if (!response.ok) {
    ...
    const isApiKeyInvalid =
      bodyLower.includes("api key not valid") || bodyLower.includes("api_key_invalid");

    if (isApiKeyInvalid || response.status === 401 || response.status === 403) {
      throw new Error(
        `GEMINI_AUTH_REJECTED|status=${response.status}|model=${model}|body=${upstreamPreview}`
      );
    }
    if (response.status === 429) {
      throw new Error(
        `GEMINI_QUOTA_EXCEEDED|status=${response.status}|model=${model}|body=${upstreamPreview}`
      );
    }
    if (response.status >= 500) {
      throw new Error(
        `GEMINI_UPSTREAM_UNAVAILABLE|status=${response.status}|model=${model}|body=${upstreamPreview}`
      );
    }
    throw new Error(
      `GEMINI_HTTP_${response.status}|model=${model}|body=${upstreamPreview}`
    );
  }
...
async function getCurrentWeatherForPakistanDistrict(district: string): Promise<{
  temp: number;
  condition: string;
  description: string;
  humidity: number;
  ...
  if (!apiKey) {
    throw new HttpsError("failed-precondition", "weather-key-missing");
  }

  const location = district.trim() || "Punjab";
  const url =
    `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(location)},PK&units=metric&appid=${encodeURIComponent(apiKey)}`;

  let response: globalThis.Response;
  try {
    response = await fetch(url, {method: "GET"});
  } catch (_) {
    throw new HttpsError("unavailable", "weather-network-unavailable");
  }

  if (!response.ok) {
    if (response.status === 401 || response.status === 403) {
      throw new HttpsError("permission-denied", "weather-provider-auth-rejected");
    }
    if (response.status === 404) {
      throw new HttpsError("not-found", "weather-location-not-found");
    }
    throw new HttpsError("unavailable", `weather-provider-http-${response.status}`);
  }
  ...
  const temp = toNumber(payload.main?.temp, 0);
  const humidity = Math.max(0, Math.round(toNumber(payload.main?.humidity, 0)));
  const windSpeed = toNumber(payload.wind?.speed, 0);
...
async function evaluateListingRiskWithGemini(params: {
  ...
  try {
    ...
    const parsed = JSON.parse((jsonTextMatch?.[0] || "{}")) as {
      riskScore?: unknown;
      flags?: unknown;
      summary?: unknown;
    };

    const riskScore = clampRisk(toNumber(parsed.riskScore, 0));
    ...
    const fallbackReason =
      errorText.includes("GEMINI_AUTH_REJECTED") ? "ai_auth_rejected" :
...
function decideListingStatus(riskScore: number): string {
  if (riskScore >= 90) return process.env.HIGH_RISK_STATUS || "blocked_or_review";
  return "pending_review";
}

async function createListingSecureInternal(uid: string, data: CreateListingRequest): Promise<{
  listingId: string;
  status: string;
  riskScore: number;
  flags: string[];
  ...
  if (sellerId !== uid) {
    throw new HttpsError("permission-denied", "sellerId must match authenticated user");
  }
  ...
  if (normalizedImageUrls.length === 0) {
    throw new HttpsError(
      "invalid-argument",
      "at least one trust photo is required",
    );
  }

  const trustLat = toNumber(
    verificationTrustPhoto.lat ?? verificationVideo.lat,
    NaN,
  );
  const trustLng = toNumber(
    verificationTrustPhoto.lng ?? verificationVideo.lng,
    NaN,
  );
  if (!Number.isFinite(trustLat) || !Number.isFinite(trustLng)) {
    throw new HttpsError(
      "invalid-argument",
      "trusted listing must include GPS lat and lng",
    );
  }

  const trustCapturedAtDate =
    toDate(verificationTrustPhoto.capturedAt) ||
    toDate(verificationVideo.capturedAt) ||
    new Date();

  const verificationVideoUrl = String(verificationVideo.url || "").trim();
  let verificationVideoDuration = toNumber(
    verificationVideo.durationSeconds,
    0,
  );
  if (verificationVideoUrl.length > 0) {
    verificationVideoDuration = toNumber(verificationVideo.durationSeconds, NaN);
    if (!Number.isFinite(verificationVideoDuration)) {
      throw new HttpsError(
        "invalid-argument",
        "mediaMetadata.verificationVideo.durationSeconds is required when video is uploaded",
      );
    }
    if (verificationVideoDuration < 5 || verificationVideoDuration > 15) {
      throw new HttpsError(
        "invalid-argument",
        "verification video duration must be between 5 and 15 seconds",
      );
    }
  }

  const verificationLat = toNumber(verificationVideo.lat, trustLat);
  const verificationLng = toNumber(verificationVideo.lng, trustLng);

  const verificationCapturedAtDate =
    toDate(verificationVideo.capturedAt) || new Date();
  const normalizedAudioUrl = (mediaMetadata.audioUrl || "").toString().trim();

  const listingDataRecord = listingData as Record<string, unknown>;
  const requestedFeaturedListing =
    listingData.featured === true ||
    listingDataRecord.promotionRequestedFeaturedListing === true;
  const requestedFeaturedAuction =
  ...
  const promotionStatus = promotionRequested
    ...
    : "none";
  const requestedPromotionCost = toNumber(listingDataRecord.featuredCost, NaN);
  ...
  const normalizedPromotionRequestedAt =
    toDate(listingDataRecord.promotionRequestedAt) || new Date();
  const marketAverage = await getMarketAverageRate(db, product, {
    originalCategory: String(listingData.category || "").trim(),
    originalSubcategory: String(
      listingDataRecord.subcategory || listingDataRecord.subCategory || "",
    ).trim(),
    ...
    subcategoryIdOrKey: String(
      listingDataRecord.subcategoryId || listingDataRecord.subcategoryKey || "",
    ).trim(),
  ...
  const ai = await evaluateListingRiskWithGemini({
    ...
    mediaMetadata: {
      verificationTrustPhoto: {
        ...
        fileSizeBytes: toNumber(verificationTrustPhoto.fileSizeBytes, 0),
      },
      verificationVideo: {
        ...
        fileSizeBytes: toNumber(verificationVideo.fileSizeBytes, 0),
      },
      imageUrls: normalizedImageUrls,
      audioUrl: normalizedAudioUrl,
    },
  ...
  const finalFlags = uniqueStrings([...heuristic.flags, ...ai.flags]);
  ...
  console.log("createListingSecureHttp: firestore write start", {
    ...
    trustPhotoUrlOrPath: String((verificationTrustPhoto as Record<string, unknown>).url ||
      (verificationTrustPhoto as Record<string, unknown>).path || ""),
    optionalMediaFields: {
      ...
      verificationTrustPhotoFileSizeBytes: toNumber(verificationTrustPhoto.fileSizeBytes, 0),
    },
  });
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (isUserBannedOrFlagged(userData)) {
      throw new HttpsError("permission-denied", "User is flagged or banned");
    }

    const usedInCurrentHour = toNumber(rateLimitSnap.get("count"), 0);
    if (usedInCurrentHour >= 5) {
      throw new HttpsError(
        "resource-exhausted",
        "Rate limit exceeded: max 5 listings per hour",
      );
    }
    ...
    tx.set(
      listingRef,
      {
        ...
        promotionRequestedAt: promotionRequested ? Timestamp.fromDate(normalizedPromotionRequestedAt) : null,
        ...
        mediaMetadata: {
          verificationTrustPhoto: {
            lat: trustLat,
            lng: trustLng,
            capturedAt: Timestamp.fromDate(trustCapturedAtDate),
            tag: (verificationTrustPhoto.tag || "").toString(),
            fileSizeBytes: toNumber(verificationTrustPhoto.fileSizeBytes, 0),
          },
          verificationVideo: {
            ...
            lng: verificationLng,
            durationSeconds: verificationVideoDuration,
            capturedAt: Timestamp.fromDate(verificationCapturedAtDate),
            tag: (verificationVideo.tag || "").toString(),
            fileSizeBytes: toNumber(verificationVideo.fileSizeBytes, 0),
          },
          imageUrls: normalizedImageUrls,
    ...
    tx.set(
      rateLimitRef,
      {
        ...
        windowStartedAt: Timestamp.fromDate(new Date(Date.UTC(
          now.getUTCFullYear(),
          now.getUTCMonth(),
          now.getUTCDate(),
          now.getUTCHours(),
  ...
  if (promotionRequested) {
    const promoEventAt = FieldValue.serverTimestamp();
...
export const createListingSecure = onCall(AI_RUNTIME_OPTIONS, async (request) => {
...
export const createListingSecureHttp = onRequest(AI_RUNTIME_OPTIONS, async (req, res) => {
  setCorsHeaders(res);
  if (req.method === "OPTIONS") {
    res.status(204).send("");
    return;
  ...
  let firestoreDocPath: string | null = null;
  let trustPhotoUrlOrPath: string | null = null;
  let optionalMediaFields: Record<string, unknown> = {};
  ...
  try {
    ...
    const listingData =
      bodyRecord.listingData && typeof bodyRecord.listingData === "object" ?
        bodyRecord.listingData as Record<string, unknown> :
        {};
    const mediaMetadata =
      bodyRecord.mediaMetadata && typeof bodyRecord.mediaMetadata === "object" ?
        bodyRecord.mediaMetadata as Record<string, unknown> :
        {};
    const verificationTrustPhoto =
      mediaMetadata.verificationTrustPhoto && typeof mediaMetadata.verificationTrustPhoto === "object" ?
        mediaMetadata.verificationTrustPhoto as Record<string, unknown> :
        {};
    const verificationVideo =
      mediaMetadata.verificationVideo && typeof mediaMetadata.verificationVideo === "object" ?
        mediaMetadata.verificationVideo as Record<string, unknown> :
        {};
    ...
    const imageUrls = Array.isArray(mediaMetadata.imageUrls) ?
      mediaMetadata.imageUrls.map((v) => String(v).trim()).filter((v) => v.length > 0) :
      [];
    trustPhotoUrlOrPath = String(
      verificationTrustPhoto.url ||
      verificationTrustPhoto.path ||
      "",
    ).trim() || null;
    optionalMediaFields = {
      verificationVideoUrl: String(verificationVideo.url || "").trim() || null,
      verificationVideoDurationSeconds: toNumber(verificationVideo.durationSeconds, 0),
      verificationVideoTag: String(verificationVideo.tag || "").trim() || null,
      verificationVideoFileSizeBytes: toNumber(verificationVideo.fileSizeBytes, 0),
    ...
    const data = rawBody as unknown as CreateListingRequest;
...
type AdminListingActionBody = {
  listingId?: unknown;
  note?: unknown;
  extensionHours?: unknown;
};

type AdminListingTransition = {
  actionType: string;
  updates: Record<string, unknown>;
  notes?: string;
  intendedStatus?: string;
  intendedAuctionStatus?: string;
};
...
function listingStatusFrom(data: Record<string, unknown>): string {
  return toLower(data.status || data.listingStatus || data.auctionStatus);
}

function auctionStatusFrom(data: Record<string, unknown>): string {
...
async function assertAdminUserWithClaims(
  uid: string,
...
function buildAdminListingNotification(actionType: string, note: string): {
...
async function notifySellerForAdminAction(
  listingId: string,
  listingData: Record<string, unknown>,
  actionType: string,
  note: string,
...
async function executeAdminListingAction(
  uid: string,
  body: AdminListingActionBody,
  buildTransition: (
    uid: string,
    listingId: string,
    listingData: Record<string, unknown>,
  ...
  ) => AdminListingTransition,
): Promise<Record<string, unknown>> {
  const listingId = requireString(body.listingId, "listingId");
  const note = String(body.note || "").trim();
  const extensionHours = Math.max(1, Math.round(toNumber(body.extensionHours, 2)));
  ...
  await notifySellerForAdminAction(
    listingId,
    listingData,
...
function ensurePost(req: Request, res: Response): boolean {
  setCorsHeaders(res);
  if (req.method === "OPTIONS") {
    res.status(204).send("");
    return false;
...
async function executeAdminHttpAction(
  req: Request,
  res: Response,
  buildTransition: (
    uid: string,
  ...
  const body = readRequestBody(req) as AdminListingActionBody;
  try {
    await assertAdminUserWithClaims(uid, decoded);
    const result = await executeAdminListingAction(uid, body, buildTransition);
    res.status(200).json(result);
  } catch (error) {
    const message = error instanceof HttpsError ? error.message : "admin-action-failed";
    console.error("adminAction: failed", {uid, error: message});
    res.status(400).json({ok: false, error: message});
  }
}

export const approveListingAdmin = onRequest({region: "asia-south1"}, async (req, res) => {
...
export const rejectListingAdmin = onRequest({region: "asia-south1"}, async (req, res) => {
  await executeAdminHttpAction(req, res, (_uid, _listingId, listingData, note) => {
    if (!note) {
      throw new HttpsError("invalid-argument", "note-required");
    }
    const status = listingStatusFrom(listingData);
    if (status === "rejected") {
      throw new HttpsError("failed-precondition", "listing-already-rejected");
    }
    return {
...
export const requestListingChangesAdmin = onRequest({region: "asia-south1"}, async (req, res) => {
  await executeAdminHttpAction(req, res, (_uid, _listingId, listingData, note) => {
    if (!note) {
      throw new HttpsError("invalid-argument", "note-required");
    }
    return {
      ...
      updates: {
        ...
        adminChangeRequestedAt: FieldValue.serverTimestamp(),
      },
      notes: note,
...
export const startAuctionAdmin = onRequest({region: "asia-south1"}, async (req, res) => {
  await executeAdminHttpAction(req, res, (uid, _listingId, listingData) => {
    ...
    if (currentStatus === "rejected") {
      throw new HttpsError("failed-precondition", "cannot-start-auction-for-rejected-listing");
    }
    if (currentAuctionStatus === "live") {
      throw new HttpsError("failed-precondition", "auction-already-live");
    }
    if (currentAuctionStatus === "cancelled" || currentAuctionStatus === "completed") {
      throw new HttpsError("failed-precondition", "cannot-restart-cancelled-or-completed-auction");
    }
    const now = new Date();
    ...
    return {
      ...
      updates: {
        ...
        bidStartTime: Timestamp.fromDate(now),
        bidExpiryTime: Timestamp.fromDate(end),
        isBidPaused: false,
...
export const pauseAuctionAdmin = onRequest({region: "asia-south1"}, async (req, res) => {
  await executeAdminHttpAction(req, res, (_uid, _listingId, listingData) => {
    const auctionStatus = auctionStatusFrom(listingData);
    if (auctionStatus !== "live") {
      throw new HttpsError("failed-precondition", `cannot-pause-from-${auctionStatus || "unknown"}`);
    }
    return {
...
export const resumeAuctionAdmin = onRequest({region: "asia-south1"}, async (req, res) => {
  await executeAdminHttpAction(req, res, (_uid, _listingId, listingData) => {
    const auctionStatus = auctionStatusFrom(listingData);
    if (auctionStatus !== "paused") {
      throw new HttpsError("failed-precondition", `cannot-resume-from-${auctionStatus || "unknown"}`);
    }
    return {
...
export const cancelAuctionAdmin = onRequest({region: "asia-south1"}, async (req, res) => {
  await executeAdminHttpAction(req, res, (_uid, _listingId, listingData, note) => {
    if (!note) {
      throw new HttpsError("invalid-argument", "note-required");
    }
    ...
    if (auctionStatus === "cancelled" || auctionStatus === "completed") {
      throw new HttpsError("failed-precondition", `cannot-cancel-from-${auctionStatus || "unknown"}`);
    }
    return {
...
export const extendAuctionAdmin = onRequest({region: "asia-south1"}, async (req, res) => {
  await executeAdminHttpAction(req, res, (_uid, _listingId, listingData, _note, extensionHours) => {
    ...
    if (auctionStatus !== "live" && auctionStatus !== "paused") {
      throw new HttpsError("failed-precondition", `cannot-extend-from-${auctionStatus || "unknown"}`);
    }
    const baseDate = toDate(listingData.bidExpiryTime) || toDate(listingData.endTime) || new Date();
    const nextDate = new Date(baseDate.getTime() + (extensionHours * 60 * 60 * 1000));
    return {
      actionType: "extend_auction",
      updates: {
        bidExpiryTime: Timestamp.fromDate(nextDate),
        endTime: Timestamp.fromDate(nextDate),
...
export const aiGenerateText = onRequest(AI_GENERATE_TEXT_RUNTIME_OPTIONS, async (req, res) => {
  setCorsHeaders(res);
  if (req.method === "OPTIONS") {
    res.status(204).send("");
    return;
...
export const weatherCurrentHttp = onRequest(AI_RUNTIME_OPTIONS, async (req, res) => {
  setCorsHeaders(res);
  if (req.method === "OPTIONS") {
    res.status(204).send("");
    return;
  ...
  try {
    ...
    const message = error instanceof HttpsError ? error.message : "weather-unavailable";
    res.status(503).json({ok: false, success: false, error: message});
  }
});

export const aiWeatherAdvisory = onRequest(AI_RUNTIME_OPTIONS, async (req, res) => {
  setCorsHeaders(res);
  if (req.method === "OPTIONS") {
    res.status(204).send("");
    return;
  ...
  const temperature = toNumber(body.temperature, 0);
  const crop = String(body.crop || "").trim();
  const prompt =
    `You are a mandi assistant. Give a short Roman-Urdu weather advisory for crop protection. ` +
...
export const aiSuggestBidRate = onRequest(AI_RUNTIME_OPTIONS, async (req, res) => {
  setCorsHeaders(res);
  if (req.method === "OPTIONS") {
    res.status(204).send("");
    return;
  ...
  const payload = {
    ...
    baseline: toNumber(body.baseline, 0),
  ...
  try {
    ...
    const suggestedRate = Math.max(0, toNumber(parsed.suggestedRate, payload.baseline));
...
const handleAiExtractCnic = async (req: Request, res: Response): Promise<void> => {
  setCorsHeaders(res);
  if (req.method === "OPTIONS") {
    res.status(204).send("");
    return;
...
export const aiExtractCnic = onRequest(
  AI_EXTRACT_CNIC_RUNTIME_OPTIONS,
  handleAiExtractCnic,
);
export const aiExtractCnicV3 = onRequest(
  AI_EXTRACT_CNIC_RUNTIME_OPTIONS,
  handleAiExtractCnic,
);

export const evaluateListingRisk = onCall(AI_RUNTIME_OPTIONS, async (request) => {
  const uid = request.auth?.uid;
  if (!uid) {
...
export const evaluateListingRiskHttp = onRequest(AI_RUNTIME_OPTIONS, async (req, res) => {
  setCorsHeaders(res);
  if (req.method === "OPTIONS") {
    res.status(204).send("");
    return;
  ...
  try {
    ...
    const message = error instanceof HttpsError ? error.message : "evaluate-listing-risk-failed";
    res.status(400).json({ok: false, error: message});
  }
});

async function suggestMarketRateInternal(
  uid: string,
  ...
  let quantity = toNumber(data.quantity, 0);
  ...
  if (listingId) {
    ...
    quantity = toNumber(listing.quantity, quantity);
    unit = String(listing.unit || listing.unitType || unit).trim();
  }

  if (!itemName) {
    throw new HttpsError("invalid-argument", "itemName or listingId is required");
  }

  const baseline = await getMarketAverageRate(db, itemName, {
  ...
  try {
    ...
    const parsed = JSON.parse(extractJsonBlock(raw)) as Record<string, unknown>;
    const aiMin = Math.round(toNumber(parsed.suggestedMin, suggestedMin));
    const aiMax = Math.round(toNumber(parsed.suggestedMax, suggestedMax));
    suggestedMin = Math.max(1, Math.min(aiMin, aiMax));
    suggestedMax = Math.max(suggestedMin + 1, Math.max(aiMin, aiMax));
    confidence = Math.max(0, Math.min(1, toNumber(parsed.confidence, confidence)));
...
export const suggestMarketRate = onCall(AI_RUNTIME_OPTIONS, async (request) => {
  const uid = request.auth?.uid;
  if (!uid) {
    throw new HttpsError("unauthenticated", "Authentication required");
  }
...
export const suggestMarketRateHttp = onRequest(AI_RUNTIME_OPTIONS, async (req, res) => {
  setCorsHeaders(res);
  if (req.method === "OPTIONS") {
    res.status(204).send("");
    return;
  ...
  try {
    ...
    const message = error instanceof HttpsError ? error.message : "suggest-market-rate-failed";
    res.status(400).json({ok: false, error: message});
  }
});
...
function normalizeTrendToken(value: unknown): string {
  return String(value || "").trim().toLowerCase();
}
...
function toTrendDirection(changePercent: number): "rising" | "falling" | "stable" {
...
function listingTimestamp(data: Record<string, unknown>): Date | null {
  return (
    toDate(data.rateDate) ||
...
function listingCrop(data: Record<string, unknown>): string {
  return String(data.cropType || data.cropName || data.itemName || data.product || "").trim();
}

function trendKey(crop: string, province: string, district: string): string {
...
function matchesTrendFilter(
  ...
  province: string,
  district: string,
  filters: {crop: string; province: string; district: string},
): boolean {
  if (filters.crop && !normalizeTrendToken(crop).includes(filters.crop)) return false;
  if (filters.province && !normalizeTrendToken(province).includes(filters.province)) return false;
  if (filters.district && !normalizeTrendToken(district).includes(filters.district)) return false;
  return true;
}

async function buildFallbackTrendMap(
  filters: {crop: string; province: string; district: string},
): Promise<Map<string, TrendAggregate>> {
  const now = Date.now();
  ...
  const listingById = new Map<string, {key: string; crop: string; province: string; district: string}>();
  ...
  for (const doc of listingSnap.docs) {
    ...
    if (!matchesTrendFilter(crop, province, district, filters)) continue;
    ...
    const rate = toNumber(data.highestBid, toNumber(data.price, 0));
    if (rate > 0 && ts) {
      if (ts >= currentStart && ts <= now) bucket.currentRates.push(rate);
      else if (ts >= previousStart && ts < currentStart) bucket.previousRates.push(rate);
    }
  ...
  for (const doc of bidsSnap.docs) {
    ...
    const ts = (toDate(data.createdAt) || toDate(data.timestamp))?.getTime();
    const bidRate = toNumber(data.bidAmount, 0);
    if (!ts || bidRate <= 0) continue;

    const bucket = trendMap.get(listing.key);
    if (!bucket) continue;

    if (ts >= currentStart && ts <= now) bucket.currentRates.push(bidRate);
    else if (ts >= previousStart && ts < currentStart) bucket.previousRates.push(bidRate);
  }

  return trendMap;
}

async function buildMandiTrendsInternal(
  data: Record<string, unknown>,
): Promise<Record<string, unknown>[]> {
  const now = Date.now();
  const oneDayMs = 24 * 60 * 60 * 1000;
  ...
  const top = Math.min(40, Math.max(1, Math.round(toNumber(data.top, 12))));
  ...
  for (const doc of mandiSnap.docs) {
    ...
    if (!ts || ts < previousStart) continue;

    const rate = toNumber(item.averagePrice, toNumber(item.average, toNumber(item.rate, 0)));
    if (rate <= 0) continue;
  ...
  for (const bucket of trendMap.values()) {
    if (bucket.currentRates.length === 0) continue;
  ...
  trends.sort((a, b) => Math.abs(toNumber(b.priceChangePercent, 0)) - Math.abs(toNumber(a.priceChangePercent, 0)));
  return trends.slice(0, top);
}

export const getMandiTrendHttp = onRequest(AI_RUNTIME_OPTIONS, async (req, res) => {
  setCorsHeaders(res);
  if (req.method === "OPTIONS") {
    res.status(204).send("");
    return;
  ...
  try {
    ...
    const message = error instanceof HttpsError ? error.message : "get-mandi-trend-failed";
    res.status(400).json({ok: false, error: message});
  }
});

async function evaluateBidRiskInternal(
  authUid: string,
  data: Record<string, unknown>,
): Promise<Record<string, unknown>> {
  const listingId = String(data.listingId || "").trim();
  const buyerUid = String(data.buyerUid || authUid).trim();
  const bidRate = toNumber(data.bidRate, 0);
  const quantity = toNumber(data.quantity, 0);
  const unit = String(data.unit || "").trim();

  if (!listingId || bidRate <= 0 || quantity <= 0 || !unit) {
    throw new HttpsError("invalid-argument", "listingId, bidRate, quantity, unit required");
  }
  if (buyerUid !== authUid) {
    throw new HttpsError("permission-denied", "buyerUid must match auth uid");
  }
  ...
  const listingPrice = toNumber(listing.price, 0);
  ...
  for (const doc of recentBidsSnap.docs) {
    const data = doc.data();
    recentRates.push(toNumber(data.bidAmount, 0));
    if (String(data.buyerId || "") === buyerUid) buyerRecentCount += 1;
  }

  const highestRecent = recentRates.length > 0
    ? Math.max(...recentRates)
    : Math.max(0, toNumber(listing.highestBid, listingPrice));
  ...
  const buyer = (buyerSnap.data() || {}) as Record<string, unknown>;
  const buyerCreatedAt = toDate(buyer.createdAt);
  ...
  let bidFlags = uniqueStrings(flags);
  ...
  try {
    ...
    const parsed = JSON.parse(extractJsonBlock(raw)) as Record<string, unknown>;
    bidRiskScore = clampRisk(toNumber(parsed.bidRiskScore, bidRiskScore));
...
export const evaluateBidRisk = onCall(AI_RUNTIME_OPTIONS, async (request) => {
...
export const evaluateBidRiskHttp = onRequest(AI_RUNTIME_OPTIONS, async (req, res) => {
  setCorsHeaders(res);
  if (req.method === "OPTIONS") {
    res.status(204).send("");
    return;
  ...
  try {
    ...
    const message = error instanceof HttpsError ? error.message : "evaluate-bid-risk-failed";
    res.status(400).json({ok: false, error: message});
  }
});

async function applyWatchersDelta({
  listingId,
  uid,
  delta,
  eventId,
  ...
  if (!normalizedListingId || !normalizedUid || !eventId.trim()) return;
  ...
  await db.runTransaction(async (tx) => {
    ...
    const current = toNumber(listing.watchersCount, 0);
...
export const onWatchlistCreated = onDocumentCreated(
  {document: "users/{uid}/watchlist/{listingId}", region: "asia-south1"},
  async (event) => {
...
export const onWatchlistDeleted = onDocumentDeleted(
  {document: "users/{uid}/watchlist/{listingId}", region: "asia-south1"},
  async (event) => {
    const uid = String(event.params.uid || "").trim();
    const listingId = String(event.params.listingId || "").trim();
...
export const onListingCreated = onDocumentCreated(
  ...
  async (event) => {
    ...
    const fraudInput: ListingForFraud = {
      sellerId,
      ...
      verificationGeo: {
        lat: toNumber((data.verificationGeo as {lat?: unknown})?.lat, 0),
        lng: toNumber((data.verificationGeo as {lng?: unknown})?.lng, 0),
      },
      verificationCapturedAt: (toDate(data.verificationCapturedAt) || new Date()).toISOString(),
    };

    const userFraudView: UserForFraud = {
      trustScore:
        typeof userData.trustScore === "number"
          ? userData.trustScore
          : undefined,
      strikes: toNumber(userData.strikes, 0),
      listingsToday: toNumber(userData.listingsToday, 0),
      isBanned: userData.isBanned === true,
      lastKnownGeo:
        userData.lastKnownGeo &&
        typeof (userData.lastKnownGeo as {lat?: unknown}).lat === "number" &&
        typeof (userData.lastKnownGeo as {lng?: unknown}).lng === "number"
          ? {
              lat: (userData.lastKnownGeo as {lat: number}).lat,
              lng: (userData.lastKnownGeo as {lng: number}).lng,
            }
          : undefined,
    };
    ...
    const fraud = await computeFraudScore({
      ...
      listing: fraudInput,
      user: userFraudView,
      highFrequency: toNumber(userData.listingsToday, 0) >= 5,
    });

    const imageUrls = Array.isArray(data.imageUrls)
      ? data.imageUrls.filter((v) => typeof v === "string") as string[]
      : [];
    ...
    for (const url of imageUrls) {
      ...
      try {
        const hash = await computeImageHashFromStoragePath(storagePath);
    ...
    if (imageHashes.length > 0) {
      ...
      for (const doc of sellerRecentSnap.docs) {
        ...
        const overlap = h.some((v: unknown) => imageHashes.includes(String(v)));
        if (overlap) {
    ...
    await snap.ref.set(
      {
        ...
        imageHashes: uniqueStrings(imageHashes),
        status,
        updatedAt: FieldValue.serverTimestamp(),
      },
      {merge: true},
...
async function markInvalidMedia(listingId: string, reason: string): Promise<void> {
  const ref = db.collection("listings").doc(listingId);
  const snap = await ref.get();
  if (!snap.exists) return;

  const existingFlags = Array.isArray(snap.get("fraudFlags"))
    ? (snap.get("fraudFlags") as string[])
    : [];
  const existingRisk = toNumber(snap.get("riskScore"), 0);

  const updatedFlags = uniqueStrings([...existingFlags, "invalid_media", reason]);
...
export const onListingMediaFinalize = onObjectFinalized(
  {region: "asia-south1", bucket: "digital-arhat.firebasestorage.app"},
  async (event) => {
    const object = event.data;
    const filePath = object.name || "";
    if (!filePath.startsWith("listings/")) return;
