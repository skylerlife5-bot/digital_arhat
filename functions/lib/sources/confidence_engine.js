 ...
 *  - otherwise → "low"
 */
function toRowConfidence(score, sourceId, freshnessStatus, flags) {
    // Hard reject: any unit violation
    if (flags.includes("unit_violation") ||
        flags.includes("critical_unit_violation")) {
        return "rejected";
    }
    // PBS SPI is trend-only — never show as numeric price
    if (flags.includes("pbs_spi_trend_only"))
        return "low";
    ...
    if (score >= 0.75 && isHighSource && isFresh)
        return "high";
    if (score >= 0.75 && isHighSource)
        return "high"; // aging but high source
    if (score >= 0.50)
        return "medium";
    return "low";
}
/** Source-level reliability bucket (independent of row freshness). */
function toSourceReliabilityLevel(sourceId) {
    const high = new Set([
        "fscpd_official",
        "lahore_official_market_rates",
        "karachi_official_price_lists",
    ...
    if (high.has(sourceId))
        return "high";
    if (sourceId.startsWith("human_verified"))
        return "medium";
    return "low";
}
function scoreConfidence(record, input) {
    const incomingFlags = input.flags ?? [];
    let score = 0.45;
    ...
    if (input.incompleteMetadata)
        score -= 0.08;
    if (input.weakLocationMatch === true)
        score -= 0.09;
    if (input.ocrWeakParse === true)
        score -= 0.08;
    // Unit violations lower score further
    if (incomingFlags.includes("unit_violation"))
        score -= 0.25;
    if (incomingFlags.includes("critical_unit_violation"))
        score -= 0.50;
    ...
    if (input.suspiciousSpike && !flags.includes("price_spike"))
        flags.push("price_spike");
    if (input.sparseData && !flags.includes("sparse_data"))
        flags.push("sparse_data");
    if (record.freshnessStatus === "stale" && !flags.includes("stale_source"))
        flags.push("stale_source");
    if (!record.commodityNameUr?.trim() && !flags.includes("no_urdu_label"))
        flags.push("no_urdu_label");
