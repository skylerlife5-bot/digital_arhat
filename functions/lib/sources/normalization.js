...
const COMMODITY_FUZZY_KEYWORDS = [
...
function normalizeCommodityLookupText(input) {
    return input
        .toLowerCase()
        .replace(/<[^>]+>/g, " ")
        .replace(/[\u2010-\u2015]/g, " ")
        ...
        .replace(/\b(tomatoes|potatoes|onions|chilies|chillies|carrots|grapes)\b/g, (word) => {
        if (word === "tomatoes")
            return "tomato";
        if (word === "potatoes")
            return "potato";
...
function fuzzyCommodityIdFromText(normalizedInput) {
    if (!normalizedInput)
        return null;
    ...
    for (const rule of COMMODITY_FUZZY_KEYWORDS) {
        for (const keyword of rule.keywords) {
            ...
            if (normalizedInput.includes(normKeyword))
                return rule.id;
            if (collapsed.includes(normKeyword.replace(/\s+/g, "")))
                return rule.id;
        }
    }
    return null;
