...
exports.TOP_25_MANDI_TARGETS = void 0;
exports.getEnabledTopPriorityMandis = getEnabledTopPriorityMandis;
const SOURCE_BY_CITY = {
...
function familyForCity(city) {
    return SOURCE_BY_CITY[city] ?? "future_city_committee_source";
}
function aliasesForCity(city) {
    const lower = city.toLowerCase();
    const aliases = new Set([city, lower]);
    if (city === "Lahore")
        aliases.add("لاہور");
    if (city === "Faisalabad")
        aliases.add("فیصل آباد");
    if (city === "Rawalpindi")
        aliases.add("راولپنڈی");
    if (city === "Multan")
        aliases.add("ملتان");
    if (city === "Bahawalpur")
        aliases.add("بہاولپور");
    if (city === "Gujranwala")
        aliases.add("گوجرانوالہ");
    if (city === "Sargodha")
        aliases.add("سرگودھا");
    if (city === "Gujrat")
        aliases.add("گجرات");
    ...
    if (city === "Khanewal")
        aliases.add("خانیوال");
    if (city === "Muzaffargarh")
        aliases.add("مظفرگڑھ");
    if (city === "Toba Tek Singh")
        aliases.add("ٹوبہ ٹیک سنگھ");
    if (city === "Kabirwala")
        aliases.add("کبیروالا");
    if (city === "Lodhran")
        aliases.add("لودھراں");
    if (city === "Chichawatni")
        aliases.add("چیچہ وطنی");
    if (city === "Jhelum")
        aliases.add("جہلم");
    if (city === "Mianwali")
        aliases.add("میانوالی");
    if (city === "Karachi")
        aliases.add("کراچی");
    if (city === "Hyderabad")
        aliases.add("حیدرآباد");
...
exports.TOP_25_MANDI_TARGETS = PRIORITY_ORDER.map((entry, index) => {
    const city = entry[0];
...
function getEnabledTopPriorityMandis() {
    return exports.TOP_25_MANDI_TARGETS.filter((item) => item.enabled);
}
